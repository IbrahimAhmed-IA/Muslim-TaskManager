...
export default function RepeatTaskModal({
  ...
  const [repeatOption, setRepeatOption] = useState<'tomorrow' | 'all' | 'custom'>('tomorrow');
  ...
  const allDays: DayOfWeek[] = ['saturday', 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday'];
  ...
  const handleRepeat = () => {
    ...
    const tasksToRepeat = taskIds.length > 0
      ? taskIds
      : tasks.filter(task => !task.completed).slice(0, 5).map(task => task.id);

    if (tasksToRepeat.length === 0) {
      toast.error("No tasks found to repeat. Please create at least one task first.");
      return;
    }

    if (repeatOption === 'tomorrow') {
      copyTasks(tasksToRepeat, tomorrow);
      toast.success(`Task${tasksToRepeat.length > 1 ? 's' : ''} repeated for tomorrow (${tomorrow})`);
    } else if (repeatOption === 'all') {
      // Copy to all days
      for (const day of allDays) {
        copyTasks(tasksToRepeat, day);
      }
      toast.success(`Task${tasksToRepeat.length > 1 ? 's' : ''} repeated for all days of the week`);
    } else if (repeatOption === 'custom' && customDays.length > 0) {
      // Copy to selected custom days
      for (const day of customDays) {
        copyTasks(tasksToRepeat, day);
      }
      toast.success(`Task${tasksToRepeat.length > 1 ? 's' : ''} repeated for ${customDays.length} selected days`);
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className={`sm:max-w-[425px] ${advancedStyles.background} ${advancedStyles.text} ${advancedStyles.border}`}>
        <DialogHeader>
          <DialogTitle className={settings.advancedMode ? 'text-white' : ''}>Repeat Tasks</DialogTitle>
        </DialogHeader>

        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            <label className={`text-sm font-medium ${settings.advancedMode ? advancedStyles.label : ''}`}>
              Repeat Options
            </label>

            <div className={`space-y-3 p-3 rounded-md ${settings.advancedMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
              ...
              {repeatOption === 'custom' && (
                <div className={`p-3 mt-2 grid grid-cols-1 gap-2 rounded-md ${settings.advancedMode ? 'bg-slate-600/50' : 'bg-gray-100'}`}>
                  {allDays.map(day => (
