...
const categoryStyles = {
  'Daily Prayers': { icon: 'FaPray', color: 'from-indigo-600 to-blue-600', bgColor: 'bg-indigo-50', darkBgColor: 'bg-indigo-900/20' },
  'Quran Reading': { icon: 'FaBook', color: 'from-emerald-600 to-teal-600', bgColor: 'bg-emerald-50', darkBgColor: 'bg-emerald-900/20' },
  'Adhkar': { icon: 'FaHandsWash', color: 'from-blue-600 to-cyan-600', bgColor: 'bg-blue-50', darkBgColor: 'bg-blue-900/20' },
  'Dhikr': { icon: 'FaHeart', color: 'from-pink-600 to-rose-600', bgColor: 'bg-pink-50', darkBgColor: 'bg-pink-900/20' },
  'Optional Prayers': { icon: 'FaPray', color: 'from-purple-600 to-violet-600', bgColor: 'bg-purple-50', darkBgColor: 'bg-purple-900/20' },
  'Fasting': { icon: 'FaMoon', color: 'from-amber-600 to-yellow-600', bgColor: 'bg-amber-50', darkBgColor: 'bg-amber-900/20' },
  'Good Deeds': { icon: 'FaHandsHelping', color: 'from-green-600 to-emerald-600', bgColor: 'bg-green-50', darkBgColor: 'bg-green-900/20' },
  'Knowledge': { icon: 'FaBook', color: 'from-cyan-600 to-sky-600', bgColor: 'bg-cyan-50', darkBgColor: 'bg-cyan-900/20' }
};
...
export default function WorshipTasksManager() {
  ...
  return (
    <div className="py-6 w-full fade-in">
      <header className={`page-header ${settings.advancedMode ? advancedStyles.headerBg : ''}`}>
      ...
      <div className="container mx-auto p-6">
        <div className={`card p-6 mb-8 ${settings.advancedMode ? advancedStyles.cardBg : ''}`}>
          <div className="flex items-center mb-6">
            ...
            <div>
              <h2 className={`text-2xl font-semibold ${settings.advancedMode ? 'text-white' : 'text-gray-800'}`}>Add Worship Tasks</h2>
              <p className={settings.advancedMode ? 'text-slate-300' : 'text-gray-500'}>Select tasks to add to your daily schedule</p>
            </div>
          </div>
              ...
              <div>
                ...
                >
                  <SelectTrigger className={`w-full capitalize input-enhanced ${
                    settings.advancedMode ? 'bg-slate-800 border-slate-600 text-white' : ''
                  }`}>
              ...
              <div>
                ...
                >
                  <SelectTrigger className={`w-full capitalize input-enhanced ${getPriorityColor(selectedPriority)} ${
                    settings.advancedMode ? 'bg-slate-800 border-slate-600' : ''
                  }`}>
                    <SelectValue placeholder="Select priority" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="low" className={settings.advancedMode ? "text-green-300" : "text-blue-500"}>
                      Low Priority
                    </SelectItem>
                    <SelectItem value="medium" className={settings.advancedMode ? "text-yellow-300" : "text-yellow-600"}>
                      Medium Priority
                    </SelectItem>
                    <SelectItem value="high" className={settings.advancedMode ? "text-red-300" : "text-red-500"}>
                      High Priority
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            {Object.entries(groupedTasks).map(([category, tasks], index) => (
              <Card
                ...
                className={`border overflow-hidden transition-all duration-300 hover:shadow-lg slide-up ${
                  settings.advancedMode ? 'shadow-md border-slate-700 bg-slate-800' : 'shadow-md border-gray-200'
                }`}
              ...
              >
                ...
                <CardContent className={`p-4 ${settings.advancedMode ? 'bg-slate-800' : ''}`}>
