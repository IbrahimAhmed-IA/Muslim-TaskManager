...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  // Header height stays at 35px but logo is bigger and text is smaller
  const headerHeight = '35px';

  return (
    <div className="flex flex-col min-h-screen">
      {/* Ultra-thin header with app title and Azan times */}
      <header
        className="w-full fixed top-0 z-30"
        style={{ height: headerHeight }}
      >
        <div
          className={`h-full page-header py-0.5 px-3 ${
        ...
        >
          <div className="flex justify-between items-center h-full">
            <div className="flex items-center space-x-1.5">
              ...
              />
              <div>
                <h1 className="text-[8px] font-semibold text-white">Muslim Task Manager</h1>
                <div className="text-white text-[6px] opacity-80">
                  {settings.advancedMode ? 'Advanced Task Management' : 'Organize your day with purpose'}
                </div>
              </div>
            </div>
      ...
      {/* Main content area - Positioned below the header with adjusted height */}
      <div className="flex min-h-screen pt-[35px]">
        {/* Sidebar - Now positioned below the header with adjusted height */}
        <div
          className={`sidebar w-20 fixed left-0 top-[35px] h-[calc(100vh-35px)] z-10 shadow-xl transition-all duration-300 ease-in-out ${
            settings.advancedMode
        ...
        >
          <div className="flex flex-col items-center justify-center mt-8 mb-12">
            ...
            <span
              className={`text-white text-xs mt-3 tracking-wide font-medium ${
                settings.advancedMode ? 'opacity-90' : 'opacity-75'
              }`}
            >
          ...
          <div className="flex flex-col items-center space-y-7">
            <Link href="/" className="flex flex-col items-center">
              ...
              <span
                className={`text-white text-xs transition-all duration-200 ${
                  pathname === '/'
                    ? settings.advancedMode
        ...
        <main
          className={`flex-1 pl-20 min-h-screen transition-colors duration-300 ${
            settings.advancedMode ? 'bg-gradient-to-b from-slate-900 to-slate-950 text-white' : 'bg-gray-50'
          }`}
        >
          {children}
        </main>
